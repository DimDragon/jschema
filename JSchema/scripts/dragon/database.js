Dragon.module(['dragon/enum'], function (Enum) {
    ...
    if (indexedDB.polyfill) {
        ...
        function fireVersionChangeEvent(request, name, oldVersion, newVersion) {
            ...
            for (var i = 0; i < database.connections.length; i++) {
                ...
                var eventc = new util.IDBVersionChangeEvent("versionchange", request, oldVersion, newVersion);
                if (conn.onversionchange) conn.onversionchange(eventc);
            }
        ...
        function storeIndex(context, index, encodedKey, isLast) {
            ...
            if (index.multiEntry && (encodedKey instanceof Array)) {
                ...
                for (var i = 0; i < encodedKey.length; i++) {
                    //fixed error from original sql.push("SELECT ?, X'" + encodedKey[i] + "', X'" + context.encodedKey + "'");
                    select.push("SELECT ?, X'" + encodedKey[i] + "', X'" + context.encodedKey + "'");
        ...
        IDBObjectStore.prototype.get = function (key) {
            console.log('IDBObjectStore.prototype.get entered');
            var encodedKeyOrRange = util.validateKeyOrRange(key);
            var request = new util.IDBRequest(this);
            var me = this;
            me.transaction._queueOperation(function (sqlTx, nextRequestCallback) {
        ...
        */
        ...
        function wrapSqlError(error) {
            ...
            return util.error("Unknown", 'Unknown', error);
        }
    }

    /*DataBase Wrapper*/
    //after polyfill applied if nessecary wrapper with exposed only part of the methods of the indexed db

    var dragonDB = window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB,
    dragonDatabase,
    initCallback,
    dragonTransaction,
    ...
    var customError = function (name, message, innerError) {
        return {
            name: name,
            message: message,
            inner: innerError
    ...
    //database manipulation methods methods
    ...
    function deleteDB(name) {
        ///<summary>
        ///		Delete databese with specified name
        ///</summary>
        ///<param name="name" type="String">
        ///		Name of the database to be deleted
        ///</param>
    	///<returns type="Promise">Return delete database promise</returns>
    	return new Promise(function (resolve, reject) {
    		dragonDatabase = null;
    		var request = dragonDB.deleteDatabase(name);
    		request.onsuccess = function (e) {
    			console.log("Database '" + name + "' has been deleted successfully.");
    			resolve(e);
    		};
    		var fn = function (e) {
    			console.error("Database '" + name + "'  deletion failed.", e);
    			reject(e);
    		};
    ...
    function initDB(clientInitCallback) {
        ///<summary>
        ///		This is client callback method used for create and modify database.
        ///ONLY here can be used methods for modification of the Database:
        ///addStore,createIndex,createStoreIndex, deleteIndex and deleteStore.
        ///This method is called only once for db version so if someone try to open a database with version higher than 
        //currently existed on client browser then this method runs.
        ///</summary>
        ///<param name="clientInitCallback" type="Function">
        ///		Function ro be executed on create/update database.
        ///</param>
        if (clientInitCallback)
            initCallback = clientInitCallback;
    }

    //transaction methods
    function openTransaction(stores) {
        ///<summary>
        ///		Open transaction that try to lock for writing of new data in specified stores.
        ///NB!!! This transaction is time limited and have different livetime for different browsers.
        ///Only one transaction can exists at given time.
        ///</summary>
        ///<param name="stores" type="Array" elementType="String">
        ///		Names of database stores to be locked for exclusive usage.
        ///</param>
        if (dragonTransaction)
            throw customError("Transaction Open Error", "One transaction is already opened. Close it before start new one!", null);

        dragonTransaction = dragonDatabase.transaction(stores, "readwrite");

        dragonTransaction.onerror = function () {
            //errors authomatically rollback transaction so we only clear our local variables 
            closeTransaction();
        };
    }
    ...
    function closeTransaction() {
        ///<summary>
        ///		Close transaction
        ///NB!!! This method is not neither commit or rollback transaction because browsers do this internaly. 
        ///This is only to help developers to start many crossing transactions that are blocked each other without of error.
        ///</summary>
        if (!dragonTransaction)
            throw customError("Transaction Close Error", "No active transaction to close!", null);
        dragonTransaction = null;
    }

    //store manipulation methods
    function createStore(name, options) {
        ///<summary>
        ///		Create object store(data table).This method can be used ONLY in initDB callback function.
        ///</summary>
        ///<param name="name" type="String">
        ///		Name of the store to be created
        ///</param>
        ///<param name="options" type="Object" optional="true">
        ///		Options object that contains 2 properties:
        // 1.keyPath (key of store table) - that is DOMString or sequence<DOMString> nullable
        // 2.autoIncrement - auto increment or not is key of the table. Default value is "false"
        ///</param>
        if (typeof (options) != "undefined")
            return dragonDatabase.createObjectStore(name, options);
        else
            return dragonDatabase.createObjectStore(name);
    }

    function deleteStore(name) {
        ///<summary>
        ///		Delete object store(data table).This method can be used ONLY in initDB callback function.
        ///</summary>
        ///<param name="name" type="String">
        ///		Name of the store to be deleted
        ///</param>
        ///<returns type="Promise">Return promise for delete operation</returns>
        return wrap(dragonDatabase.deleteObjectStore(name));
    }

    function getStore(store) {
        ///<summary>
        ///		Get store by its name.
        ///NB!!! This method is exposed only for test purpoces so avoid using it
        ///</summary>
        ///<param name="store" type="String">
        ///		Name of the store
        ///</param>
        ///<returns type="Object">Return store specified by its name</returns>
        var dbStore;
        if (!dragonTransaction) {
            var transaction = dragonDatabase.transaction(store, "readonly");
            dbStore = transaction.objectStore(store)
        }
        else {
            dbStore = dragonTransaction.objectStore(store);
        }
        return dbStore;
    }
    ...
    function countStoreRecords(store) {
        ///<summary>
        ///		Get store records count.
        ///</summary>
        ///<param name="store" type="String">
        ///		Name of the store
        ///</param>
        ///<returns type="Promise">Return promise to stored in store records count</returns>
        var dbStore;
        if (!dragonTransaction) {
            var transaction = dragonDatabase.transaction(store, "readonly");
            dbStore = transaction.objectStore(store);
        ...
        return wrap(dbStore.count());
    }

    //index manipulation methods
    function createStoreIndex(store, indexName, fieldName, options) {
        ///<summary>
        ///		Create index key for store. This method can be used ONLY in initDB callback function.
        ///</summary>
        ///<param name="store" type="String">
        ///		Name of the store
        ///</param>
        ...
        ///		Name of the field used for index. This is DOMString or sequence<DOMString> used for key path of the index
        ///</param>
        ///<param name="options" type="Object">
        ///		Options used for creation of index
        ///Options are :
        ///1.unique - boolean property specified unique or not is index;
        ///2.multiEntry - boolean property specified multientry for created index;
        ///</param>
        ///<returns type="Promise">Return promise for create index operation</returns>
        if (typeof (options) != "undefined")
            return wrap(store.createIndex(indexName, fieldName, options));
        else
            return wrap(store.createIndex(indexName, fieldName));
    }

    function deleteIndex(store, name) {
        ///<summary>
        ///		Delete object store index. This method can be used ONLY in initDB callback function.
        ///</summary>
        ///<param name="store" type="String">
        ///		Name of the store
        ///</param>
        ...
        ///		Name of the index
        ///</param>
        ///<returns type="Dragon.Promise">Return promise for delete index operation</returns>
        return wrap(store.deleteIndex(name));
    }

    function getIndexes(store) {
        ///<summary>
        ///		Get currently existing indexes of specified store.
        ///</summary>
        ///<param name="store" type="String">
        ///		Name of the store
        ///</param>
        ///<returns type="DOMStringList">Return existing indexes</returns>
        var dbStore;
        if (!dragonTransaction) {
            var transaction = dragonDatabase.transaction(store, "readonly");
            dbStore = transaction.objectStore(store);
        ...
        return dbStore.indexNames;
    }

    //records methods
    function insertRecord(store, data, key) {
        ///<summary>
        ///		Insert record in specified store table
        ///</summary>
        ///<param name="store" type="String">
        ///		Name of the store
        ///</param>
        ///<param name="data" type="Object">
        ///		Data value to be stored in store table
        ///</param>
        ///<param name="key" type="Object" optional="true">
        ///		keyPath (key of store table) - that is DOMString or sequence<DOMString>. 
        ///</param>
        ///<returns type="Promise">Return promise for insert record operation</returns>
        var dbStore;
        if (!dragonTransaction) {
            var transaction = dragonDatabase.transaction(store, "readwrite");
            dbStore = transaction.objectStore(store);
        }
        else {
            dbStore = dragonTransaction.objectStore(store);
        ...
        return new Promise(function (resolve, reject) {
        	...
        	try {
        		if (key)
        			request = dbStore.add(data, key);
        		else
        			request = dbStore.add(data);
    ...
    function selectRecord(store, key) {
        ///<summary>
        ///		Get record from specified store table. Method is succesfull even no one record is returned, so check record after gettting. 
        ///</summary>
        ///<param name="store" type="String">
        ///		Name of the store
        ///</param>
        ...
        ///		keyPath (key of store table) - that is DOMString or sequence<DOMString>. 
        ///</param>
        ///<returns type="Promise">Return promise for select record operation.</returns>
        var dbStore;
        if (!dragonTransaction) {
            var transaction = dragonDatabase.transaction(store, "readwrite");
            dbStore = transaction.objectStore(store);
    ...
    function deleteRecord(store, key) {
        ///<summary>
        ///		Delete record from specified store table. Method is succesfull even no one record is deleted. 
        ///</summary>
        ///<param name="store" type="String">
        ///		Name of the store
        ///</param>
        ...
        ///		keyPath (key of store table) - that is DOMString or sequence<DOMString>. 
        ///</param>
        ///<returns type="Promise">Return promise for delete record operation.</returns>
        var dbStore;
        if (!dragonTransaction) {
            var transaction = dragonDatabase.transaction(store, "readwrite");
            dbStore = transaction.objectStore(store);
        ...
        return wrap(dbStore.delete(key));
    }

    var CursorDirection = new Enum({ 'next': 'next', 'nextunique': 'nextunique', 'prev': 'prev', 'prevunique': 'prevunique' });
    var CursorRange = {};

    CursorRange.only = function (value) {
        return window.IDBKeyRange.only(value, value, false, false);
    };

    CursorRange.lowerBound = function (lower, open) {
        return window.IDBKeyRange.lowerBound(lower, open || false, true);
    };

    CursorRange.upperBound = function (upper, open) {
        return window.IDBKeyRange.upperBound(upper, true, open || false);
    };

    CursorRange.bound = function (lower, upper, lowerOpen, upperOpen) {
        return window.IDBKeyRange.bound(lower, upper, lowerOpen || false, upperOpen || false);
    };

    //cursor methods
    function selectCursor(store, range, direction) {
        ///<summary>
        ///		Select records from specified store in specified range and direction. 
        ///</summary>
        ///<param name="store" type="String">
        ///		Name of the store
        ///</param>
        ///<param name="range" type="Object" optional="true">
        ///		This parameter define range of values that must be returned. Range can have following values:
        ///1.Numeric value - This value is used as key starting point.
        ///2.One of CursorRange methods:
        ///     - CursorRange.only(value) - return only element with this value if exists
        ///     - CursorRange.lowerBound(lower, open) - return elements greater from specified lower. 
        ///         "Open" parameter determine included or not is lower element. Default for "open" is false that means included lower element.
        ///     - CursorRange.upperBound(upper, open) - return elements lower from specified upper. 
        ///         "Open" parameter determine included or not is upper element. Default for "open" is false that means included upper element.
        ///     - CursorRange.bound(lower,upper, openLower, openUpper) - return elements bettween lower and upper.
        ///         "openLower" and "openUpper" parameters determine included or not is corresponding value element. Default for "openLower" and "openUpper" is false that means included value element.
        ///</param>
        ///<param name="direction" type="Dragon.Database.CursorDirection" optional="true">
        ///		This is direction for selecting and returning records.
        ///</param>
    	///<returns type="Promise">When resolved returns array of objects with following properties: key,primaryKey and value</returns>
    	return new Promise(function (resolve, reject) {
    		var dbStore;

    		if (!dragonTransaction) {
    			var transaction = dragonDatabase.transaction(store, "readwrite"); //chrome not return record when set to readonly
    			dbStore = transaction.objectStore(store);
    		}
    		else {
    			dbStore = dragonTransaction.objectStore(store);
    		...
    		var rq = range ? dbStore.openCursor(range, direction) : dbStore.openCursor(),
				result = [];
    ...
    function selectIndexCursor(store, indexName, range, direction) {
        ///<summary>
        ///		Select records from specified store using one of its indexes in specified range and direction. 
        ///</summary>
        ///<param name="store" type="String">
        ///		Name of the store
        ///</param>
        ///<param name="indexName" type="String">
        ///		Name of the store index
        ///</param>
        ///<param name="range" type="Object" optional="true">
        ///		This parameter define range of values that must be returned. Range can have following values:
        ///1.Object value - This value is used as starting point.
        ///2.One of CursorRange methods:
        ///     - CursorRange.only(value) - return only element with this value if exists
        ///     - CursorRange.lowerBound(lower, open) - return elements greater from specified lower. 
        ///         "Open" parameter determine included or not is lower element. Default for "open" is false that means included lower element.
    	...
    	return new Promise(function (resolve, reject) {
    		...
    		var rq = range ? dbStore.index(indexName).openCursor(range, direction || CursorDirection.next) : dbStore.index(indexName).openCursor(),
				result = [];
    ...
    return {
        ...
        getStore: getStore,
        createIndex: createStoreIndex,
        deleteIndex: deleteIndex,
        getIndexes: getIndexes,
        addRecord: insertRecord,
        getRecord: selectRecord,
        removeRecord: deleteRecord,
        getCursor: selectCursor,
        indexCursor: selectIndexCursor,
        getRecordCount: countStoreRecords,
        CursorRange: CursorRange,
        CursorDirection: CursorDirection,
        _namespace: 'Dragon.Database'
    };
});